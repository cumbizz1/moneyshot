...
export class VideoFileService {
  public async convert2Mp4(
    ...
    try {
      const fileName = `${StringHelper.randomString(5)}-${StringHelper.getFileName(filePath, true)}.mp4`;
      const toPath = options?.toPath ? join(`${options?.toPath}/`, fileName) : join(StringHelper.getFilePath(filePath), fileName);
